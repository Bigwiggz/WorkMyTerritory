...
{
    public class CampaignAddValidator: AbstractValidator<CampaignAddSpecialEventsViewModel>
    {
        private readonly IEnumerable<CampaignSpecialEvents> _campaigns;

        public CampaignAddValidator(IEnumerable<CampaignSpecialEvents> campaigns)
        {
            ...
            RuleFor(c => c.SpecialCampaignName)
                .Must(IsCampaignUnique).WithMessage("Campaign Entered is not Unique.");
        }
        //Need to fix adding (SpecialCampaignName,SpecialCampaignEndDate,SpecialCampaignStartDate,CampaignActive,FKCongregationID)
        //Custom Validator
        public bool IsCampaignUnique(CampaignAddSpecialEventsViewModel editedCampaign, string newValue)
        {
